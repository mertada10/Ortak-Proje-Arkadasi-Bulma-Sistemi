    ...
    foreach ($allUsers as $candidate) {
        ...
        if ($myTeamId) {
            $q = $pdo->prepare("
                ...
                LIMIT 1
            ");
            $q->execute([$myTeamId, $candidate["id"]]);
            if ($q->fetch()) { continue; }
        }
        ...
        if (strtolower(trim($currentUser["department"] ?? "")) === strtolower(trim($candidate["department"] ?? ""))) {
            $score += 15;
        }

        $mySkills = array_filter(array_map("trim", explode(",", strtolower($currentUser["skills"] ?? ""))));
        $hisSkills = array_filter(array_map("trim", explode(",", strtolower($candidate["skills"] ?? ""))));
        $score += count(array_intersect($mySkills, $hisSkills)) * 5;

        $myInterests = array_filter(array_map("trim", explode(",", strtolower($currentUser["interests"] ?? ""))));
        $hisInterests = array_filter(array_map("trim", explode(",", strtolower($candidate["interests"] ?? ""))));
...
if ($search != "") {
    $stmt = $pdo->prepare("
        ...
        WHERE projects.title LIKE ? OR projects.description LIKE ? OR projects.required_skills LIKE ?
...
<div class="main-container">
    ...
    <div class="search-container">
        <div class="search-box">
            ...
            <input type="search" id="searchInput" placeholder="Proje ara..." autocomplete="off">
        </div>
    </div>
