                    ...
                    <?php if (count($project_members) > 0): ?>
                        <?php foreach ($project_members as $member): ?>
                            <div class="project-member-item">
                                ...
                                <?php if ($is_owner): ?>
                                    <form method="post" class="project-member-remove-form" data-project-id="<?= (int)$project["id"] ?>" data-member-id="<?= (int)$member["id"] ?>" onsubmit="return confirm('Bu kullanıcıyı projeden çıkarmak istediğinize emin misiniz?');">
            ...
            <div class="author-card">
                ...
                <?php if ($is_owner): ?>
                    <div class="owner-actions">
                        <?php if ($ad_is_active): ?>
                            <a href="post-ad.php?project=<?= (int)$project["id"] ?>" class="btn-advert ad-edit-link">
                                <i class="fa-solid fa-pen"></i> İlanı Düzenle
                            </a>
                            <form method="post" class="ad-remove-form" data-project-id="<?= (int)$project["id"] ?>" onsubmit="return confirm('Bu ilanı kaldırmak istediğinizden emin misiniz? Proje silinmeyecek.');">
                                <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                        ...
                        <?php else: ?>
                            <a href="post-ad.php?project=<?= (int)$project["id"] ?>" class="btn-advert ad-give-link">
                                <i class="fa-solid fa-bullhorn"></i> İlan Ver
                            </a>
                        <?php endif; ?>
                        
                        <?php if ($from === "projects"): ?>
                            <form method="post" class="project-delete-form" data-project-id="<?= (int)$project["id"] ?>" onsubmit="return confirm('DİKKAT! Bu projeyi tamamen silmek istediğinize emin misiniz? Proje, ilan ve projeye bağlı tüm istekler silinecek. Bu işlem geri alınamaz.');">
                                <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                                <input type="hidden" name="id" value="<?= (int)$project["id"] ?>">
                ...
                <?php if (!$is_owner && $already_in_project): ?>
                    <div class="owner-actions">
                        <form method="POST" class="project-leave-form" data-project-id="<?= (int)$project["id"] ?>" onsubmit="return confirm('Bu projeden ayrılmak istediğinize emin misiniz?');">
                            <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                            <input type="hidden" name="project_id" value="<?= (int)$project["id"] ?>">
