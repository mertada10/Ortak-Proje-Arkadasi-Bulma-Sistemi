...
$stmt = $pdo->prepare("SELECT id, name, surname, username, department, email, phone, skills, interests, about, profile_image, last_seen FROM users WHERE id = ?");
    ...
    <div class="profile-card">
        <div class="profile-sidebar">
            ...
            <?php if ($viewing_own_profile): ?>
                <form id="avatar-form" action="/teammate-finder/api/upload-avatar.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="profile-row profile-projects">
                ...
                <div class="profile-projects-content">
                                ...
                                <?php foreach ($userProjects as $project): ?>
                                    <div class="profile-project-item">
                                        <div class="profile-project-info">
                                            ...
                                            <p class="profile-project-desc">
                                                <?= htmlspecialchars(mb_substr($desc, 0, 140)) ?><?= mb_strlen($desc) > 140 ? "..." : "" ?>
                                            </p>
                                        </div>

                                        <a href="project-detail.php?id=<?= (int)$project["id"] ?>" class="btn-detail">
                                            İncele
                                        </a>
                                    </div>
                                <?php endforeach; ?>
                                ...
                                <?php foreach ($userJoinedProjects as $jproject): ?>
                                    <div class="profile-project-item">
                                        <div class="profile-project-info">
                                            ...
                                            <p class="profile-project-desc">
                                                <?= htmlspecialchars(mb_substr($jdesc, 0, 140)) ?><?= mb_strlen($jdesc) > 140 ? "..." : "" ?>
                                            </p>
                                        </div>

                                        <a href="project-detail.php?id=<?= (int)$jproject["id"] ?>" class="btn-detail">
                                            İncele
                                        </a>
                                    </div>
                                <?php endforeach; ?>
                    ...
                    <?php if (count($userProjects) === 0 && count($userJoinedProjects) === 0): ?>
                        <p class="profile-projects-empty">
                            Henüz bir projesi bulunmuyor.
                        </p>
                    <?php endif; ?>
                </div>
            </div>
                    ...
                    <div class="profile-actions">
                        ...
                        <?php else: ?>
                            ...
                            <small class="action-note">
                                Takıma davet göndermek için önce bir takım oluşturmanız gerekiyor.
                            </small>
                        <?php endif; ?>
                    </div>
...
<div class="modal-overlay" id="inviteModal" aria-hidden="true">
    <div class="modal-box" role="dialog" aria-modal="true" aria-labelledby="inviteModalTitle">
        ...
        <div class="modal-header">
            ...
            <button type="button" class="modal-close" onclick="closeInviteModal()" aria-label="Kapat">
                &times;
            </button>
        </div>

        <p class="modal-subtitle">
            Bu kullanıcıyı hangi takımınıza davet etmek istiyorsunuz?
        </p>

        <form id="inviteForm" method="POST" action="../api/send-team-request.php">
            <input type="hidden" name="user_id" id="inviteUserId">
            <input type="hidden" name="team_id" id="inviteTeamId">

            <div class="invite-team-list">
                <?php if (empty($currentUserTeams)): ?>

                    <p class="invite-teams-empty">
                        Davet gönderebileceğiniz takımınız yok. Lider veya yardımcı lideri olduğunuz bir
                        <a href="create-team.php">takım oluşturun</a>.
                    </p>
                ...
                <?php else: ?>
                    ...
                    <?php foreach ($currentUserTeams as $inviteTeam): ?>
                        <div class="invite-team-option-wrap" data-team-id="<?= (int)$inviteTeam["id"] ?>">
                            <button
                                type="button"
                                class="invite-team-option"
                                data-team-id="<?= (int)$inviteTeam["id"] ?>"
                                onclick="selectInviteTeam(this)"
                            >
                                <i class="fa-solid fa-users"></i>

                                <span>
                                    <?= htmlspecialchars($inviteTeam["team_name"]) ?>
                                </span>

                                <span class="invite-team-pending">
                                    Beklemede
                                </span>
            ...
            <div class="form-group">
                <label for="inviteNote">
                    <i class="fa-solid fa-note-sticky" style="margin-top: 8px;"></i>
                    Not (İsteğe Bağlı)
                </label>
                ...
                <textarea
                    ...
                    rows="4"
                    maxlength="500"
                    placeholder="Davet isteğinize bir not ekleyiniz."
                ></textarea>

                <small class="team-action-note">
                    En fazla 500 karakter.
                </small>
            </div>

            <div class="modal-footer">
                <button type="button" class="edit-btn" onclick="closeInviteModal()">
                    Vazgeç
                </button>

                <button
                    type="button"
                    class="edit-btn invite-send-btn"
                    id="sendInviteBtn"
                    onclick="sendInvite()"
                    disabled
                >
...
function isInviteTeamDisabled(teamId) {
    return !!invitePendingTeams[teamId] || !!inviteMemberTeams[teamId];
}
...
function refreshInviteTeams() {
    document.querySelectorAll(".invite-team-option-wrap").forEach(function(wrap) {
        ...
        var isPending = !!invitePendingTeams[teamId];
        var isMember = !!inviteMemberTeams[teamId];
...
function selectInviteTeam(btn) {
    ...
    document.querySelectorAll(".invite-team-option-wrap").forEach(function(wrap) {
        wrap.classList.toggle(
            "selected",
            wrap.getAttribute("data-team-id") === selectedInviteTeamId
        );
    });
...
function sendInvite() {
    ...
    var url = '../api/send-team-request.php'
        + '?user=' + encodeURIComponent(inviteTargetUserId)
        + '&team=' + encodeURIComponent(selectedInviteTeamId)
