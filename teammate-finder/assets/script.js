...
/* ==== AJAX BİLDİRİM SİSTEMİ ==== */
(function () {
    const notificationEndpoint = "/teammate-finder/api/notifications.php";
    ...
    function loadNotifications() {
        ...
        .then(response => {
            if (!response.ok) {
                throw new Error("Bildirim API hatası: " + response.status);
            }
        ...
        .then(data => {
            ...
            updateBadge("messagesNotificationBadge", data.messages);
            updateBadge("requestsNotificationBadge", data.requests_total);
            updateBadge("profileNotificationBadge", data.total);
        })
        .catch(error => {
