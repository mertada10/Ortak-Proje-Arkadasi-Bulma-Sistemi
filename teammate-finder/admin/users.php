...
if ($search !== "") {
    $stmt = $pdo->prepare("
        ...
        FROM users 
        WHERE name LIKE ? OR surname LIKE ? OR username LIKE ? OR email LIKE ?
        ORDER BY id DESC
    ");
    $searchTerm = "%{$search}%";
        ...
        <main class="main-content">
            ...
            <div class="content-section">
                ...
                <div class="section-header">
                    <h2><i class="fa-solid fa-users"></i> Toplam Kullanıcı (<?= count($users) ?>)</h2>
                </div>
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                ...
                                                <?php if ((int)$u["id"] !== (int)$_SESSION["user_id"]): ?>
                                                    <a href="users.php?action=delete&id=<?= $u["id"] ?>" class="btn btn-delete" onclick="return confirm('Bu kullanıcıyı silmek istediğinize emin misiniz?')" title="Kullanıcıyı Sil">
